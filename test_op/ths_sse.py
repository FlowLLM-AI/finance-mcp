...
async def process_single_stock(
    ...
    async with semaphore:  # 信号量控制并发数
        ...
        try:
            logger.info(f"[{index}/{total}] 调用 {tool_name} 处理股票 {code}")
            ...
            if not result.is_error:
                ...
                logger.error(f"✗ 错误 {code}: {error_msg}, 耗时 {elapsed_time:.2f}秒")
                
        except Exception as e:
            import traceback
            elapsed_time = (datetime.now() - start_time).total_seconds()
            logger.error(f"✗ 失败 {code}: {e}, 耗时 {elapsed_time:.2f}秒")
